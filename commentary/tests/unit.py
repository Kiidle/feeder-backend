...
from commentary.models import Commentary, Feed
...
class CommentaryTestCase(TestCase):
    def setUp(self):
        ...
        Commentary.objects.create(
            text="Test commentary", feed=feed, author=commentaryAuthor
        )

    def test_commentary_text(self):
        commentary = Commentary.objects.get(text="Test commentary")
        self.assertEqual(
            commentary.text, "Test commentary", "Commentary Text not Equal"
        )
    ...
    def test_commentary_create(self):
        ...
        feedAuthor = User.objects.create(
            username="feed.creator", email="feed.creator@test.test"
        )
        feed = Feed.objects.create(text="Test feed", author=feedAuthor)
        commentaryAuthor = User.objects.create(
            username="commentary.creator", email="commentary.creator@test.test"
        )
        Commentary.objects.create(
            text="Test commentary", feed=feed, author=commentaryAuthor
        )
    ...
    def test_commentary_update(self):
        ...
        self.assertEqual(
            updated_commentary.text,
            "Updated test commentary",
            "Commentary Text not updated",
        )
    ...
    def test_censores_text_on_commentary(self):
        ...
        commentary = Commentary.objects.create(
            text="Arsch du Hure", author=self.feedAuthor, feed=feed
        )
        self.assertEqual(commentary.text, "***** du ****")
        self.assertEqual(self.feedAuthor.warns.count(), 1)

        commentary = Commentary.objects.create(
            text="Hallo", author=self.feedAuthor, feed=feed
        )
